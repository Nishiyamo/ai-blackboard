...
function handleCustomTldrawPaste(
  editor: Editor,
  ...
  const onlyCopiedFrame =
    onlyCopiedShape?.type === "frame"
      ? (onlyCopiedShape as TLFrameShape)
      : null;
  ...
  if (
    !newlyPastedFrame ||
    !editor.isShapeOfType<TLFrameShape>(newlyPastedFrame, "frame")
  )
    return;

  const siblingIds = editor.getSortedChildIdsForParent(
    newlyPastedFrame.parentId
  );
  ...
  for (const sibling of siblingBounds) {
    ...
    if (
      sibling.bounds.minY > pastedBounds.maxY ||
      sibling.bounds.maxY < pastedBounds.minY
    )
      continue;
